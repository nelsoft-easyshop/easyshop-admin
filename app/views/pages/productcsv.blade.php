...
<div id="mainsection">
        ...
        <div class='registration_form' id="registration_form">
            {{ Form::open(array('url' => 'productcsv','enctype'=>'multipart/form-data', "id" => "uploadData")) }}
                ...
                {{ Form::open(array('url' => "#",'enctype'=>'multipart/form-data','id'=>'uploadphoto','method'=>'post')) }}
                    <legend>Upload Product Images</legend>
                        ...
                        @foreach($result as $data => $key)
                           <input name="filenames[]" type="hidden" value="{{$key}}"/>
                           <input name="time[]" type="hidden" value="{{round(microtime(true) * 1000) + $data}}"/>
                        @endforeach
                    <div class="form-group">
