 ...
 /**
    ...
    public function getUserAccountsToRefund($dateFrom, $dateTo,$username = null)
    {
        ...
        $returnedOrders = $query->get(['es_member.username',
                                       'es_member.email', 
                                       'es_member.contactno', 
                                        DB::raw("IF( es_order_product.buyer_billing_id != '0', es_bank_info.bank_name, '') as bank_name"), 
                                        DB::raw("IF( es_order_product.buyer_billing_id != '0', es_billing_info.bank_account_name, '') as account_name"), 
                                        DB::raw("IF( es_order_product.buyer_billing_id != '0', es_billing_info.bank_account_number, '') as account_number"),
                                        DB::raw('SUM(es_order_product.net) as net')
                                    ]);
