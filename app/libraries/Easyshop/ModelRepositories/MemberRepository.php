...
{    
    ...
    public function getUserAccountsToRefund($username, $dateFrom, $dateTo)
    {
        ...
        $query = DB::table('es_order_product')->join('es_member','es_order_product.seller_id', '=', 'es_member.id_member');
        $query->join('es_order','es_order_product.order_id', '=', 'es_order.id_order');
        ...
        $returnedOrders = $query->get(['es_member.username', 'es_member.email', 'es_member.contactno', DB::raw('SUM(es_order_product.net) as net')]);
